...
type LocaleValue = { locale: Locale; setLocale: (locale: Locale) => void; t: (source: string) => string };
...
export function LocaleProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const skipped = (element: Element | null) =>
      !!element?.closest("[data-no-translate], script, style, svg, [contenteditable]");
  ...
  return <LocaleContext.Provider value={{ locale, setLocale, t: (source) => translateText(source, locale) }}>{children}</LocaleContext.Provider>;
}
