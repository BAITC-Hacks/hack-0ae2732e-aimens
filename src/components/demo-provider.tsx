...
const TEAM_STORAGE_KEY = "praktika-demo-team-actor";
...
function isDemoActor(value: string | null): value is string {
  // Custom team ids are generated by the server. The API still validates that
  // the selected team exists before returning any profile data.
  return (
    value !== null &&
    (demoActors.has(value) || /^team-[0-9a-f-]{36}$/iu.test(value))
  );
}

type DemoContextValue = {
  actor: string;
  lastTeamActor: string;
...
export function DemoProvider({ children }: { children: ReactNode }) {
  ...
  const [lastTeamActor, setLastTeamActor] = useState("team-1");
  ...
  useEffect(() => {
    const restoreActor = window.setTimeout(() => {
      try {
        ...
        const storedTeam = window.sessionStorage.getItem(TEAM_STORAGE_KEY);
        if (isDemoActor(storedTeam) && storedTeam !== "business") {
          setLastTeamActor(storedTeam);
        } else if (
          storedActor &&
          storedActor !== "business" &&
          isDemoActor(storedActor)
        ) {
          setLastTeamActor(storedActor);
        }
      } catch {
  ...
  function setActor(next: string) {
    ...
    try {
      ...
      if (next !== "business") {
        window.sessionStorage.setItem(TEAM_STORAGE_KEY, next);
      }
    } catch {
      /* Persisting the selector is optional for restricted browsers. */
    }
    if (next !== "business") setLastTeamActor(next);
