...
```sh
git clone --branch codex/sanalink-integrated https://github.com/BAITC-Hacks/hack-0ae2732e-aimens.git
...
**4. Отправьте предложение.** Переключитесь в режим **«Студент»** и выберите **Nomad Labs** в меню профиля. Откройте созданную задачу и отправьте:
...
**6. Подтвердите результат.** В режиме Nomad Labs отправьте первый результат: «Подготовлен прототип прогноза по синтетическим данным», ссылка `https://example.com/result`. Вернитесь в бизнес и нажмите **«Подтвердить результат · +10»**.
...
**Проверено для изменения AI-конструктора 23 сентября 2026:** 75 модульных тестов и 5 браузерных тестов `tests/e2e/builder-ai.spec.ts` прошли; TypeScript и ESLint — без ошибок. Команды выше позволяют отдельно воспроизвести полный набор проверок, включая сборку и перезапуск. В автоматических AI-тестах внешний ответ подменяется; доступность реального OpenAI API проверяется отдельно.
...
**Репозиторий:** [BAITC-Hacks / hack-0ae2732e-aimens](https://github.com/BAITC-Hacks/hack-0ae2732e-aimens/tree/codex/sanalink-integrated).
