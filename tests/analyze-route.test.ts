...
describe("preliminary task rating", () => {
  it("uses the original description as preliminary context", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/analyze", {
        ...
        body: JSON.stringify({
          description:
            "Сеть кафе каждый вечер списывает непроданную выпечку",
          card: emptyCard,
        }),
      }),
    );
    ...
    expect(result.preliminaryReadiness.score).toBe(10);
    expect(
