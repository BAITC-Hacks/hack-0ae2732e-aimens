...
| `src/server/ai.ts` | Промпт, Responses API, структурированный ответ, проверка цитат, локальные вопросы |
| `src/app/api/` | Node.js HTTP-обработчики, валидация запросов и ответы об ошибках |
| `src/components/demo-provider.tsx` | Текущая демороль, серверный снимок, выполнение действий, состояние загрузки и сообщения |
...
JSON-входы проверяются Zod; тело запроса ограничено 65 000 символами. Демороль задаётся заголовком `x-demo-actor`: `business` или `team-1`…`team-5`. Этот заголовок моделирует поведение ролей, аутентификации в MVP нет.
...
Проект использует один локальный сервер. Нет настоящих аккаунтов, отдельных владельцев бизнеса, загрузки файлов, чата и production-инфраструктуры. Текущий демопрофиль сбрасывается при полной перезагрузке, несохранённый редактор теряется. Сохранённые карточки и результаты остаются в SQLite, избранное сохраняется в браузере. Реальный платный запрос OpenAI в рамках доработки не выполнялся; тесты внешнего адаптера используют подмену SDK.
