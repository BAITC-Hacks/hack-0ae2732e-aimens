...
| `src/domain/demo-data.ts`                      | Пять исходных описаний, пять опубликованных карточек и пять команд                                    |
...
Главная `/` знакомит с площадкой; `/catalog` содержит полный каталог. Конструктор `/tasks/new`, карточка `/tasks/[id]` и редактор `/tasks/[id]/edit` связаны с кабинетами `/business` и `/team`. `/teams` показывает команды и `/teams/new` создаёт отдельный командный профиль. `/guide` объясняет правила.
...
- `tasks`: идентификатор, JSON карточки, исходное описание, компания, даты создания, подтверждения и публикации, JSON финальной оценки. В демо владелец всех задач — единый профиль бизнеса.
...
JSON-входы проверяются Zod; тело POST-запроса ограничено 65 000 байтами при чтении потока. AI маршруты ограничены общей частотой процесса. Демороль задаётся заголовком `x-demo-actor`: `business`, `team-1`…`team-5` или созданный динамический id команды. Этот заголовок моделирует поведение ролей, аутентификации в MVP нет.
...
| `POST /api/suggest-topic` | `{ description }` → тема из списка, уверенность и основание               |
| `POST /api/review-task`   | `{ rawDescription, card }` → финальные баллы и одноразовый токен проверки |

| `type` действия    | Дополнительные поля                                                     | Роль                                                         |
| ------------------ | ----------------------------------------------------------------------- | ------------------------------------------------------------ |
| `save-task`        | `id?`, `card`, `rawDescription`, `confirmed`, `publish`, `reviewToken?` | Бизнес; публикация требует действующую оценку текущей версии |
| `create-team`      | `name`, `interests`, `iconKey`                                          | Создать отдельный командный профиль                          |
...
Публикация отправляет подтверждённые поля одним действием и требует одноразовый токен анализа, связанный с хешем исходного текста и полей. Для опубликованной задачи сервер запрещает черновое перезаписывание. Карточка, дата подтверждения и оценка записываются вместе. Порог готовности не ограничивает публикацию. Снятие с публикации не входит в MVP.
