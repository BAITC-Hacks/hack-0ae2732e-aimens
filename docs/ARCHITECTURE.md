...
Серверный снимок в `DemoProvider` привязан к текущему демопрофилю и поколению загрузки: смена роли очищает прежние данные, устаревшие ответы не применяются. Успешный ответ POST подтверждает сохранение независимо от следующего GET. Ошибка обновления показывается отдельно с повтором загрузки, сохраняя локальные состояния форм. Обычное обновление снимка не размонтирует экран.

Конструктор хранит текущий и максимальный посещённый шаг отдельно. Возврат по шагам не запускает AI повторно; изменение полей требует нового подтверждения. Первый результат редактируется через существующий серверный upsert до подтверждения бизнесом; новая запись и дополнительные баллы при исправлении не создаются.
...
## Интеграция дополнительных возможностей

В итоговой `main` сохранены создание демокоманды (`create-team`, `/teams/new`), двенадцать тем и подбор темы через `/api/suggest-topic`. Созданная команда хранится в SQLite и может отправлять предложения на общих основаниях.

`/api/review-task` даёт необязательный разбор содержания. Сервер проверяет схему, выдаёт токен на 15 минут, привязанный к версии карточки, и при валидном токене сохраняет разбор в отдельной колонке `quality_review`. Колонка добавляется миграцией при открытии существующей базы. Отсутствие или истечение токена не блокирует публикацию. `computeReadiness` остаётся единственным источником официального рейтинга.

Русский интерфейс — исходный. `LocaleProvider` применяет словари казахского и английского к текстам интерфейса; выбор языка сохраняется в браузере. Значения полей и пользовательские тексты не переводятся. Локализация не переводит произвольные ответы внешней модели.
